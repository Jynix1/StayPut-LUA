...
local discordRPClib
local discordRPCStatus = "ready"

local function getDiscordLibraryCandidates()
    local candidates = { "discord-rpc.dll", "discord-rpc" }
    ...
    if love and love.filesystem and love.filesystem.getSourceBaseDirectory then
        ...
        if baseDir then
            candidates[#candidates + 1] = baseDir .. "/discord-rpc.dll"
            candidates[#candidates + 1] = baseDir .. "\\discord-rpc.dll"
            candidates[#candidates + 1] = baseDir .. "/discord-rpc"
            candidates[#candidates + 1] = baseDir .. "\\discord-rpc"
        end
    end
...
local ok, err = pcall(function()
    for _, candidate in ipairs(getDiscordLibraryCandidates()) do
        local okLoad, loadErr = pcall(function()
            discordRPClib = ffi.load(candidate)
        end)

        if okLoad and discordRPClib then
            return true
        end
    end

    error("Could not load discord-rpc from any known path")
end)

if not ok then
    discordRPClib = nil
    discordRPCStatus = err or "Unable to load discord-rpc"
end
...
discordRPC.available = discordRPClib ~= nil
discordRPC.status = discordRPCStatus
...
local ready_proxy
local disconnected_proxy
local errored_proxy
local joinGame_proxy
local spectateGame_proxy
local joinRequest_proxy

if discordRPClib then
    ready_proxy = ffi.cast("readyPtr", function(request)
    if discordRPC.ready then
        discordRPC.ready(unpackDiscordUser(request))
    end
end)

    disconnected_proxy = ffi.cast("disconnectedPtr", function(errorCode, message)
        if discordRPC.disconnected then
            discordRPC.disconnected(errorCode, ffi.string(message))
        end
    end)

    errored_proxy = ffi.cast("erroredPtr", function(errorCode, message)
        if discordRPC.errored then
            discordRPC.errored(errorCode, ffi.string(message))
        end
    end)

    joinGame_proxy = ffi.cast("joinGamePtr", function(joinSecret)
        if discordRPC.joinGame then
            discordRPC.joinGame(ffi.string(joinSecret))
        end
    end)

    spectateGame_proxy = ffi.cast("spectateGamePtr", function(spectateSecret)
        if discordRPC.spectateGame then
            discordRPC.spectateGame(ffi.string(spectateSecret))
        end
    end)

    joinRequest_proxy = ffi.cast("joinRequestPtr", function(request)
...
function discordRPC.initialize(applicationId, autoRegister, optionalSteamId)
    if not discordRPC.available then
        return false
    end
...
getmetatable(discordRPC.gcDummy).__gc = function()
    ...
    if disconnected_proxy then
        disconnected_proxy:free()
    end
    if errored_proxy then
        errored_proxy:free()
    end
    if joinGame_proxy then
        joinGame_proxy:free()
    end
    if spectateGame_proxy then
        spectateGame_proxy:free()
    end
    if joinRequest_proxy then
